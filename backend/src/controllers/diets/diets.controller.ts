...
export const getAllLogic = async (
	userId: string,
	calories: string,
	ingredient: string
) => {
...
export const getAll = async (req: any, res: any) => {
	try {
		const { calories, ingredient } = req.query;
...
export const getAllFavsLogic = async (
	userId: string,
	calories: string,
	ingredient: string
) => {
...
export const postDiet = async (req: any, res: any) => {
	try {
		const { name, calories, ingredients, macros, micros } = req.body;
		...
		for (let i = 0; i < ingredients.length; i++) {
			...
			await Diet.agregarIng(
				ingId,
				ingredients[i].ingrediente,
				ingredients[i].cantidad,
				ingredients[i].unidad,
				dietId
			);
		}
