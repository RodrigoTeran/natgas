...
export default class Diet {
	...
	static async findAll({
		...
		if (calories != "0") {
			myArray.push(
				`${JSON.stringify(calories).slice(
					1,
					JSON.stringify(calories).length - 1
				)}`
			);
		} else {
			myArray.push("%");
		...
		if (ingredient !== undefined) {
			myArray.push(
				`${JSON.stringify(ingredient).slice(
					1,
					JSON.stringify(ingredient).length - 1
				)}%`
			);
		} else {
			myArray.push("%");
